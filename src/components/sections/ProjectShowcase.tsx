...
const PhoneMockup = ({ title, desc, tag, rotate = 0, zIndex = 10, offset = 0, active = false }: any) => (
  <motion.div 
    initial={{ opacity: 0, y: 100, rotate: rotate }}
    whileInView={{ opacity: 1, y: offset, rotate: rotate }}
    viewport={{ once: false }}
    transition={{ type: "spring", stiffness: 40, damping: 20, delay: active ? 0.2 : 0.4 }}
    style={{ zIndex: zIndex }}
    className="absolute bottom-[-10%] w-[280px] md:w-[320px] aspect-[9/19] bg-[#050505] border-[10px] border-[#1a1a1a] rounded-[3.5rem] shadow-[0_50px_100px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col"
  >
    {/* Status Bar */}
    ...
    <div className="flex-1 p-6 flex flex-col">
       <div className="flex justify-between items-center mb-10">
          ...
          <div className="flex gap-2">
             ...
             <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center border border-white/10">
                <div className="w-4 h-[1px] bg-white/40 mb-1" />
                <div className="absolute w-4 h-[1px] bg-white/40 mt-1" />
             </div>
          </div>
       </div>
       ...
       <div className="flex-1 flex flex-col items-center text-center">
          ...
          <button className={`w-full py-4 rounded-3xl font-black text-[11px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-transform hover:scale-[0.98] ${active ? 'bg-gradient-to-r from-orange-600 to-orange-400 text-white shadow-[0_10px_30px_rgba(234,88,12,0.3)]' : 'bg-transparent border border-white/10 text-white/40'}`}>
             {tag} <ArrowRight size={14} className="stroke-[3]" />
          </button>
          
          {!active && (
             <button className="w-full mt-4 py-4 rounded-3xl font-black text-[10px] uppercase tracking-[0.1em] border border-white/10 text-white/20">
                Try {title} AI
             </button>
          )}
  ...
  return (
    <section className="w-full bg-[#030303] pt-40 pb-60 px-6 md:px-12 lg:px-20 relative overflow-hidden flex flex-col items-center">
      ...
      <div className="z-30 text-center md:text-right md:w-full md:max-w-7xl mb-40">
        <h1 className="text-[48px] md:text-[72px] font-[900] text-white tracking-[-0.07em] flex items-center justify-center md:justify-end gap-4 leading-none">
          Founder <span className="text-white/20">of</span> 
          <span className="font-['Cormorant_Garamond'] italic font-normal text-transparent bg-clip-text bg-gradient-to-r from-[#eb3678] to-[#f97316] ml-2 tracking-tight">Rune</span>
        </h1>
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-[400px] bg-gradient-to-t from-orange-500/5 to-transparent blur-[120px] pointer-events-none" />
    </section>
  );
};
